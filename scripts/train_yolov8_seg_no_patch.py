...
def main():
    ...
    # Train Model
    print("Starting YOLOv8 Training (NO MONKEY-PATCH)...")
    ...
    model_name = MODEL_WEIGHTS.replace('.pt', '')  # e.g., 'yolov8m-seg'
    folder_name = f"{model_name}-{timestamp}"
    
    print(f"Model will be saved to: {os.path.join(MODELS_DIR, folder_name)}")

    # Train WITHOUT overlap_mask (this avoids the bug entirely)
    results = model.train(
        data=YAML_PATH,
        epochs=EPOCHS,
        imgsz=IMG_SIZE,
        ...
        overlap_mask=False,  # DISABLE to avoid the IndexError bug
